...
# 根据输入的key生成key列表
def Get_KeyList(data):
    data_list = []
    for ch in data:
        data_list.append(ord(ch.upper()) - 65)
    return data_list


# 加密函数
def Encrypt(plaintext, data_list):
    ciphertext = ""
    ...
    for ch in plaintext:  # 遍历明文
        ...
        if ch.isalpha():  # 明文是否为字母,如果是,则判断大小写,分别进行加密
            if ch.isupper():
                ciphertext += letter_list[(ord(ch) - 65 + data_list[i]) % 26]
                i += 1
            else:
                ciphertext += letter_list[(ord(ch) - 97 + data_list[i]) % 26].lower()
                i += 1
